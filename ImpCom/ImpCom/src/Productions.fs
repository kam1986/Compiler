...
let inline ToCommonRepresentation (Productions productions) =
    productions
...
let AddDiviation n rules actions (Productions productions) =
...
let addEndOfParse (Productions productions) =
...
let GetErrorMessages (Production (p, rules)) =
...
let MakeNFAOfproduction count nfa state actions (Production (production, rules)) =
...
let rec nullable acc last symbols =
...
let Nullable (Productions productions) =
    let Iteration last = 
...
let rec fline nullable fst first line =
    match line with
    | [] -> first
...
let rec flines nullable fst first lines =
    match lines with
    | [] -> first
    | line :: lines -> 
...
let First nullable ((Productions productions) as p) =
...
let GetFollowConstraintByFirst first follow productions =
...
let GetFollowConstraintByFollow nullable follow productions =
...
let Follow (Productions productions as p) =
